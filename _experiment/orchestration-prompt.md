...
| Sprint journal | Read, create, and update the designated Notion Sprint Journal page | Required at Discovery commitment, Delivery start, and sprint close; auth or write failure is logged and blocks that status transition |
| Issue tracking | Read, create, project-assign, and update Linear issues | Required at Discovery commitment, Delivery start, and sprint close; auth or write failure is logged and blocks that status transition |
...
### Runner attribution — derived, not self-reported

Added after Sprint 18 (26 Aug 2026), when a released lease's already-completed work was attributed first to one runner on an unverifiable live-chat recollection, then to a different cause (a second scheduled task colliding with the first) that was itself never fully confirmed either. `runner` and `session` had been treated as free text a claiming session could type from belief rather than a fact it could demonstrate.

1. **`session` is provider-namespaced from an observable fact about the current runtime, not chosen freely.** At claim time (step 5 above), prefix the session id with a tag derived from something actually true of this runtime — for example, a runner with the Cowork-specific tool surface and workspace-shell bridge available prefixes `claude-`; a runner bootstrapped via `AGENTS.md` with Codex's own distinct local tool surface prefixes `codex-`. Set `runner` from the same derived fact at the same moment, not retyped from memory later in the run.
2. **A session may only assert attribution for its own actions.** When resuming a released lease (step 4 above), report the inherited `runner`, `session`, and `last_checkpoint` exactly as found, marked *as recorded by the prior session; not independently verified by this one*. Do not restate, confirm, or "correct" a prior attribution on the strength of a live-chat recollection, a plausible-sounding explanation, or anything short of a reproducible check against the actual repository/filesystem state. If the record looks wrong, log the discrepancy as a new, separately dated entry — never rewritten into the original.
3. **Corrections are appended, never rewritten in place**, consistent with the standing rule that historical and incident records are not rewritten. A wrong attribution stays visible alongside its correction, because the failure mode itself is part of the record this experiment is collecting.
...
**Step 6 — Log and publish the sprint commitment.**

**6a — Local commitment.** Log the sprint commitment to `decision-log.md` using the schema below.

**6b — Visibility sync (mandatory before releasing the Discovery lease).** Create or update both external records immediately; do not wait for Delivery or close:

- **Linear:** one canonical issue titled `Sprint <N>: <objective summary>`, assigned to the **TurbulentGround project** and set to **Todo**. Its description must state `Discovery complete — ready for Delivery; Delivery has not started`, the committed objective, user outcome, measured baseline, acceptance criteria, out-of-scope boundaries, and the local sprint-log path. Save the Linear identifier and URL in both the sprint log and the Notion page. Search before creating so a retry updates the existing issue rather than producing a duplicate.
- **Notion:** one canonical `Sprint <N>` page in the **Sprint Journal** data source, set to **Status: In progress** and **Deployed: No**. Include the committed objective, current phase (`Discovery complete — ready for Delivery`), vote, baseline, acceptance checklist, out-of-scope boundaries, and the Linear URL. Search before creating so a retry updates the existing page rather than producing a duplicate.
- **Read-after-write verification:** fetch both records after writing. Confirm the Linear issue is in `Todo` **and assigned to the TurbulentGround project** (workspace-only issues do not appear on the project board); confirm the Notion page is in the Sprint Journal with `Status: In progress`. Log `VISIBILITY SYNC — Sprint <N> — Linear <id> Todo / Notion In progress — verified` to `decision-log.md`.
- If either record cannot be created, updated, project-assigned, linked, or verified, log `CAPABILITY FAILURE — Sprint visibility — <runner> — <Notion | Linear>: <error>`. Do not claim Discovery complete and do not release the lease as a successful handoff; leave a resumable checkpoint identifying the failed system and surface it to Graham.
...
- **Before implementation begins, sync the phase externally.** Fetch the canonical Linear and Notion records created at Discovery. Move Linear from **Todo** to **In Progress** and update the Notion Sprint Journal page's current-phase text to `Delivery in progress` while retaining `Status: In progress` and `Deployed: No`. Read both back and log `VISIBILITY SYNC — Sprint <N> — Delivery in progress — verified`. If either record is missing, recreate it from the committed objective and link the pair before implementation. A write or verification failure is a `CAPABILITY FAILURE — Sprint visibility` and blocks implementation; status may not drift silently from the work actually underway.
...
| Restricted-unlink filesystem | Git's own internal writes (create → write → commit into place) succeed under `.git/`. Explicit unlink of a lock-named file fails unconditionally. **A plain `mv`/rename of a lock-named file also fails unconditionally** — including a file this same process created itself, moments earlier, in the same shell. Rename-away is not a workaround on this profile; it fails exactly as often as unlink does | Do not attempt to clear lock-named files at all. Use `ps` as the sole concurrency signal, then go straight to the temporary-index plumbing route below. Judge success by exit code and final SHA comparison only |

The restricted-unlink profile was confirmed in the Cowork filesystem bridge on 24 August 2026. The profile belongs to the runtime path, not to Claude: a future Codex or third-party runtime showing the same behavior uses the same route, and a runner on a standard filesystem does not use the fallback merely because another session of the same provider once needed it.

**Correction, 24 August 2026 (v3.16).** v3.13's row above claimed renaming a lock out of its expected path works on this profile. Direct, repeated testing that day disproved it: `mv` on an existing `.git/index.lock`/`.git/HEAD.lock` failed with `Operation not permitted`, and — the decisive case — so did `mv` on a lock-named scratch file created by that same process seconds earlier, in the same shell, never touched by any other session. If rename genuinely worked on this profile, that last case would have succeeded; it didn't. So the four-path rename-away pre-check that used to run before the plumbing route is removed below: on this profile it fails every time a lock-named file exists for *any* reason — including one git itself just created and is about to clean up after a fully successful operation — and the old script treated that failure as `CONCURRENT SESSION SUSPECTED`, which is wrong. A failed rename here is a property of the mount, never a concurrency signal, and must never gate progress or trigger escalation on its own.

What the same testing confirmed still works: a plain read-only command (`git status`) completed successfully immediately after creating and failing to clean up its own `.git/index.lock`, printing the warning but returning exit 0 with correct output. The temporary-index plumbing route below was separately verified end-to-end the same day — `write-tree`/`commit-tree`/`update-ref`/`push` all succeeded, confirmed by SHA match against `origin/main` and a `READY` Vercel deployment, with `.git/HEAD.lock` and `.git/index.lock` present and un-removable throughout. Not directly tested that day: whether a *plain* `git add`/`git commit` (without the custom index) would itself succeed-with-warning the same way `git status` did, or hit a fatal `File exists` error, when a lock from a genuinely separate, already-dead session is the one sitting there. Until that's observed, treat the plumbing route as the proven-safe default on this profile rather than assuming the plain porcelain path is equally safe.

For the restricted-unlink profile, these details are load-bearing: check for a live Git process first, using `ps` as the **only** concurrency signal — never a failed lock-file rename or unlink, which fails unconditionally on this profile regardless of whether anything else is running. If process inspection itself is unavailable or fails, fail closed and escalate — never interpret an unavailable concurrency check as “no process found.” Match the Git executable name exactly; a substring search for `git` is unsafe (the Codex validation drill falsely matched the unrelated CalDigit utility). Then leave any existing lock-named files exactly where they are and go straight to the plumbing route — do not spend a step trying to clear them first.
...
# No rename-away pre-check on this profile (v3.16 — removed after direct testing showed
# rename fails unconditionally here, even on a file this process created itself seconds
# earlier, so a failed rename is never a usable concurrency signal on this profile, only
# a property of the mount). Any existing .git/HEAD.lock, .git/index.lock,
# .git/refs/heads/main.lock, .git/refs/remotes/origin/main.lock are left exactly as
# found. Proceed directly to the plumbing route below regardless of their presence.
```
...
`write-tree`, `commit-tree`, `update-ref`, or fetch may print `warning: unable to unlink ...` after succeeding in the restricted profile. Judge success by exit code and the final SHA comparison, not by the warning text. A live Git process (per the `ps` check), a failed command exit in the plumbing route itself, or a final SHA mismatch triggers the owner-notification and halt rule above. A failed `rm` or `mv` on a lock-named file, by itself, never does — on this profile it fails unconditionally and proves nothing about concurrency.
...
sprint to Notion and, where relevant, log any resulting work items in Linear:
  ...
  - If either write fails, log `CAPABILITY FAILURE — <Sprint journal | Issue tracking> — <runner> — <Notion | Linear>: <error>` to
    ...
    committed as real.
  - **Close-time read-after-write verification is mandatory.** Fetch the canonical Linear issue and Notion Sprint Journal page after the updates. Confirm Linear is **Done** only when the sprint actually shipped; otherwise use the truthful blocked/cancelled state. Confirm Notion's `Status`, `Deployed`, and `Commit` agree with production verification and `_experiment/sprint-state.json`. Log `VISIBILITY SYNC — Sprint <N> — close state verified`. If either differs, correct it before Step 8; if correction fails, log `CAPABILITY FAILURE — Sprint visibility` and do not mark the sprint cleanly complete.
...
| Version | Date | Change |
|---|---|---|
| 3.18 | 26 Aug 2026 | **Runner attribution made a derived fact, not a self-report.** Added after Sprint 18's Wednesday delivery produced an unresolvable attribution puzzle: work completed under a released lease was first attributed to Codex on an unverifiable live-chat recollection, then that explanation itself competed with a second, better-evidenced one (`turbulentground-daily-sprint` and `turbulentground-midweek-sprint` firing 0.344s apart against the same checkout) — and neither could be confirmed after the fact because `session`/`runner` were free text. New "Runner attribution" subsection under Sprint lock: session ids are now provider-namespaced (`claude-...` / `codex-...`) from an observable fact about the claiming runtime rather than chosen freely; a session may only assert attribution for its own actions and must report an inherited runner/checkpoint as unverified rather than confirming or correcting it from recollection alone; corrections are appended as new dated entries, never rewritten into the original. Also folds in the substance of the separately-drafted `LLM-AGNOSTIC-MIGRATION-PROPOSAL.md`, most of which turned out to already be live via v3.13–v3.17 (provider-neutral `CLAUDE.md`/`AGENTS.md` bootstraps, outcome-based capability preflight, detected-filesystem-profile git recovery, single lease-expiry rule, `runner`/`phase`/`lease_expires`/`last_checkpoint` state fields) — that document is retained as a historical record of the migration's reasoning but is no longer a pending implementation. Outstanding and NOT addressed here: whether Codex has its own automatic recurring trigger against this project remains unverified from this side and needs checking on Codex's own scheduling surface. |
| 3.17 | 25 Aug 2026 | **Lifecycle visibility made transactional across Linear and Notion.** Discovery Step 6 now requires creating/updating and read-after-write verifying a Linear issue in the TurbulentGround project at Todo plus a Notion Sprint Journal page at In progress before a successful handoff. Delivery start moves Linear to In Progress and records `Delivery in progress` in Notion before implementation. Close verifies both systems against the actual deployment and sprint state before completion. Missing project assignment, missing pages, duplicate creation, connector failures, and stale status are now explicit `CAPABILITY FAILURE — Sprint visibility` conditions rather than silent omissions. Added after Sprint 18's issue existed workspace-wide but was invisible on the project board, while its Notion page was absent until Graham asked twice. |
| 3.16 | 24 Aug 2026 | **Git-lock rename-away pre-check removed — the workaround it depended on doesn't hold.** v3.13 claimed the restricted-unlink profile lets a lock be renamed out of its expected path even though it can't be unlinked. Direct, repeated testing this session disproved it: `mv` failed identically to `rm` on every lock-named file tried, including one this same process created itself seconds earlier in the same shell — the decisive case, since it rules out a genuinely dead prior session as the explanation. The old script treated a failed rename as `CONCURRENT SESSION SUSPECTED` and escalated; on this profile that fires on nearly every run, since a plain `git status` alone leaves a `.git/index.lock` behind that no later command can remove. Removed the four-path rename-away step from the restricted-unlink route entirely. `ps` is now the sole concurrency signal on this profile; sessions proceed straight to the temporary-index plumbing route regardless of what lock-named files are sitting in `.git/`, and judge success only by exit code and final SHA match against `origin/main` — both re-verified working end-to-end this session (`31de1a9`, Vercel `READY`). Not yet tested: whether a *plain* `git add`/`git commit` (without the custom index) merely warns-and-succeeds the way `git status` did, or fails fatally, against a lock left by a genuinely separate dead session — flagged as an open question for whichever session next hits that exact case, rather than assumed either way. |
