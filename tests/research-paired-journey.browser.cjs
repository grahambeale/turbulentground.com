...
(async()=>{const b=await chromium.launch({headless:true});const p=await b.newPage();const errors=[],writes=[];p.on('pageerror',e=>errors.push(e.message));let saved={instrumentVersion:'phase3-v5-2026-09-25-examples',startedAt:'2026-09-25T00:00:00Z',consent:{takingPart:true},context:{},pairResponses:{d1:{contribution:3,contribution_context:'Synthetic judgement note'}}};
...
for(let i=0;i<12;i++){
 ...
 for(let j=0;j<2;j++){const row=rows.nth(j);const text=await row.locator('p').first().evaluate(e=>e.firstChild.textContent.trim());assert.equal(text,expected[i][j+1]);assert((await row.locator('.statement-help-toggle').innerText()).includes('See an example'));await row.locator('.statement-help-toggle').click();assert(await row.locator('.statement-help').isVisible());assert((await row.locator('.statement-help').innerText()).length>30);await row.locator('.scale-btn').nth(j===0?3:1).click();await row.locator('textarea').fill('Synthetic pair '+i+' statement '+j);}
