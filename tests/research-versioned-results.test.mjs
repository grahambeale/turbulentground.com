...
import {INSTRUMENTS,LEGACY_VERSION,PAIRED_VERSION,PREVIOUS_PAIRED_VERSION} from '../api/_research-instruments.js';
...
for(const [version,file] of [[LEGACY_VERSION,'legacy-v3.html'],[PAIRED_VERSION,'index.html']]){
 ...
 const canonical=actual.map(d=>({key:d.key,name:d.name,statements:[d.s1,d.s2].map(s=>({text:s.text,field:s.field,help:s.help}))}));
 assert.deepEqual(INSTRUMENTS[version],version===PAIRED_VERSION?canonical:canonical.map(d=>({...d,statements:d.statements.map(({help,...s})=>s)})));
}
assert.equal(INSTRUMENTS[PAIRED_VERSION].length,12);assert.equal(INSTRUMENTS[PAIRED_VERSION].flatMap(d=>d.statements).filter(s=>s.help).length,24);
assert(!buildEmailHtml('',{}, {cohortSize:0,domains:{}},'fictional',PREVIOUS_PAIRED_VERSION).includes(PAIRED_VERSION));
...
const own=buildEmailHtml('',pairs,{cohortSize:14,domains:bench.domains},'fictional',PAIRED_VERSION);assert(!own.includes('Benchmark: 3.1'));assert(own.includes('own answers only'));
...
console.log('PASS: exact frozen version wording, examples and storage mapping; v4/v5 rendering; per-item benchmark floor; safe own-answer fallback; distinct missingness and escaped names');
