...
let sends=[],version=v3,cohortCalls=0,compatibleCount=15;
...
version=v4;res=response();await results({method:'POST',body:{token:'synthetic2'}},res);assert.equal(res._status,200);assert.equal(sends.length,2);assert.equal(cohortCalls,2);assert(sends[1].html.includes('Current study benchmark 5.0 / 5'));assert(!sends[1].html.includes('Current study benchmark 2.0 / 5'));assert(sends[1].html.includes(v4));assert(sends[1].html.includes('I can decide when to use AI in my work.'));assert(!sends[1].html.includes("I'm comfortable explaining how I use AI at work."));
compatibleCount=14;res=response();await results({method:'POST',body:{token:'synthetic-own'}},res);assert.equal(res._status,200);assert.equal(sends.length,3);assert(sends[2].html.includes('own answers only'));assert(!sends[2].html.includes('Current study benchmark 5.0'));assert(!sends[2].html.includes('Current study benchmark 2.0'));
