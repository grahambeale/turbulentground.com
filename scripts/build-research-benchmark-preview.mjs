...
for(const state of ['building','available','partial']) {
  ...
  writeFileSync(`research/benchmark-preview/${state}.html`,buildEmailHtml('',pairs,benchmark,'synthetic-preview',version).replace(/[ \t]+$/gm,''));
}
...
<body><header><small>SYNTHETIC DEMONSTRATION · NO PARTICIPANT DATA OR EMAIL SENDING</small><h1>Your study comparison</h1><p>Review how the results email explains unavailable and available comparisons. Earlier question versions remain separate following the context audit.</p><div role="group" aria-label="Comparison scenario"><button data-state="building" aria-pressed="true">14 answers · still building</button><button data-state="available" aria-pressed="false">15 answers · available</button><button data-state="partial" aria-pressed="false">Mixed availability</button></div><p id="description" aria-live="polite">Each statement has 14 eligible synthetic answers, below the existing minimum.</p></header><iframe title="Synthetic research results email" src="benchmark-preview/building.html"></iframe><script>
...
document.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('button').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));document.getElementById('description').textContent=descriptions[b.dataset.state];document.querySelector('iframe').src='benchmark-preview/'+b.dataset.state+'.html';}));
</script></body></html>`);
