...
export default async function saveSubmission(req,res,data){
 ...
 if(process.env.RESEARCH_FEEDBACK_CAPTURE_ENABLED!=='true'&&!(process.env.VERCEL_ENV==='preview'&&process.env.VERCEL_GIT_COMMIT_REF==='codex/feedback-capture-preview'))return res.status(503).json({error:'Feedback capture is not enabled yet'});
 ...
 try{
  ...
  const existing=await fetch(endpoint+'?'+query,{headers});if(!existing.ok)throw Error('Read failed');const found=await existing.json();
  ...
  const write=await fetch(endpoint,{method:'PATCH',headers,body:JSON.stringify({performUpsert:{fieldsToMergeOn:[F.id]},records:[{fields:prepared.fields}]})});if(!write.ok)throw Error('Write failed');
 ...
 }catch{return res.status(502).json({error:'Could not save feedback. Your draft is still here; please retry.'})}
}
